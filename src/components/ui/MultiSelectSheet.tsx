...
export const MultiSelectSheet = (props: SheetProps<"multi-select-sheet">) => {
  const { payload } = props;
  ...
  const groupedItems = useMemo(() => {
    return filteredItems.reduce((acc, item) => {
      const group = item.group || t("common.others");
  ...
  const handleApply = () => {
    payload?.onSelect(selectedIds);
    props.sheetId && SheetManager.hide(props.sheetId);
  };

  const renderItem = ({ item }: { item: (typeof filteredItems)[0] }) => {
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text variant="h3">{payload?.title}</Text>
